...
export class CharacterDetailComponent implements OnInit {
  ...
  async getCharacterAbout() {
    if (!this.id) return;
    (await this.characterService.getCharacterDetailsById(this.id)).subscribe(
      async (result: any) => {
        let lastSeenDate;
        if (!result) {
        ...
        let epNumber = this.helper.returnSplitUrlApi(
          result.episode[result.episode.length - 1]
        );
        (await this.characterService.getLastEpisode(+epNumber)).subscribe(
          async (episode: any) => {
            if (!episode) {
            ...
            this.aboutDescription = await this.aboutDescriptionContructor(
              result,
              lastSeenDate
            );
          },
  ...
  async aboutDescriptionContructor(
    result: {
      name: string;
      gender: string;
      species: string;
    ...
    else lastSeen = 'Last seen in ' + lastSeenDate;
  ...
  async originDescriptionConstructor(urlOrigin: string) {
    ...
    (await this.characterService.getCharacterLocationById(+originId)).subscribe(
      async (origin: any) => {
        if (!origin) {
        ...
        if (!origin.residents.length) this.originResidents = 0;
      },
      (error) => {
        let msgError = this.helper.returnMsgToRequest(error);
        this.helper.showToastMsg(msgError.friendlyMessage, '', 4500);
  ...
  async locationDescriptionConstructor(urlLocation: string) {
    ...
    (
      await this.characterService.getCharacterLocationById(+locationId)
    ).subscribe(
      async (location: any) => {
        if (!location) {
          this.helper.showToastMsg('Location not found!', '', 4500);
        ...
        if (!location.residents.length) this.locationResidents = 0;
      },
      (error) => {
        let msgError = this.helper.returnMsgToRequest(error);
        this.helper.showToastMsg(msgError.friendlyMessage, '', 4500);
